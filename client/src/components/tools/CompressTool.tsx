...
import { downloadBytes, formatFileSize } from '@/lib/utils/downloadUtils'
...
export default function CompressTool() {
  ...
  const getQualityDescription = () => {
    switch (quality) {
      case 'low':
        return 'Smallest file size, lowest quality'
      case 'high':
    ...
    <div className="space-y-6">
      {!file ? (
        ...
        <div className="file-item animate-fade-in p-4">
          <div className="w-10 h-12 bg-gradient-to-br from-purple-50 to-purple-100 
                          dark:from-purple-950/50 dark:to-purple-900/30
                          rounded-lg flex items-center justify-center flex-shrink-0 
                          border border-purple-100 dark:border-purple-900/50">
            <svg className="w-5 h-6 text-purple-600 dark:text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-medium text-gray-800 dark:text-gray-200 truncate">{file.name}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">{formatFileSize(originalSize)}</p>
          </div>
          <button
            onClick={handleReset}
      ...
      {file && (
        <div className="space-y-4 animate-fade-in">
          <div>
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Compression quality</p>
      ...
      {status === 'done' && (
        <div className="success-banner">
          ...
          <p className="text-sm text-green-700 dark:text-green-300 font-medium">PDF compressed and downloaded successfully!</p>
        </div>
      )}

      {status === 'error' && (
        <ErrorBanner errors={['Failed to compress PDF. Please check that the file is a valid PDF.']} />
      )}
