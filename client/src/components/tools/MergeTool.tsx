...
function SortableItem({ file, onRemove }: { file: UploadedFile; onRemove: (id: string) => void }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`file-item ${isDragging ? 'file-item-dragging' : ''} animate-fade-in`}
    >
      <button
        {...attributes}
        {...listeners}
      ...
      <div className="w-8 h-10 bg-gradient-to-br from-primary-50 to-primary-100 
        ...
        <svg className="w-4 h-5 text-primary-600 dark:text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3 animate-fade-in">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              {upload.files.length > 1 && <span className="text-gray-400 dark:text-gray-500 ml-1">(drag to reorder)</span>}
            </p>
            <button 
      ...
      {status === 'done' && (
        <div className="success-banner">
          ...
          <p className="text-sm text-green-700 dark:text-green-300 font-medium">PDF merged and downloaded successfully!</p>
        </div>
      )}
      ...
      {upload.files.length < 2 && upload.files.length > 0 && (
        <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1 animate-fade-in">
